...
class TableSpiderDuke(scrapy.Spider):

    """
    Spider for websites formatted like Duke's and Holy Cross's page
    """
    name = 'soccerSpider4'
    ...
    def parse_list(self, response):
        ...
        for player in players:
            ...
            yield playerItem
            pages_to_visit = response.xpath('//*[@id="q_season"]/option/@value').extract()
            ...
            for i in range(1, num_pages_to_visit): #skipping index 0 because that is the starting page already parsed. Only parsing 10 years if applicable
                if(i < len(pages_to_visit)):
                    if school_url == 'www.goduke':
                        urlToVisit = ('http://www.goduke.com/SportSelect.dbml?' +
                                      'DB_OEM_ID=4200&SPID=1833&SPSID=22446&KEY=&Q_SEASON=' + pages_to_visit[i])
                    elif school_url == 'goholycross':
                        urlToVisit = ('https://goholycross.com/SportSelect.dbml?' +
                                      'DB_OEM_ID=33100&SPID=174208&SPSID=1020214&KEY=&Q_SEASON=' + pages_to_visit[i])
    ...
    def reference_index(self, school_url, attribute):
        ...
        return str(TableSpiderDuke.INDEX[school_url][attribute])
