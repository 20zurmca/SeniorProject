...
class SoccerSpider(scrapy.Spider):
    ...
    CURRENT_ROSTER_YEAR = int(get_current_roster_year().split('-')[0]) #i.e 2018
    ...
    def parse_list(self, response):
        ...
        players    = response.xpath(SoccerSpider.PLAYER_LIST_VIEW) #list of <li/> nodes for each player
        rosterYear = response.xpath(SoccerSpider.ROSTER_YEAR_FROM_LIST_VIEW).extract_first().split(" ") #format is a list of strings containing roster year e.g
        ...
        for player in players: #time to parse
            ...
            name                = player.xpath(SoccerSpider.NAME_FROM_LIST_VIEW).extract_first() #string with a single name e.g 'John Doe'
            position            = player.xpath(SoccerSpider.POSITION_LIST_VIEW).extract_first() #string with a single position, unstriped e.g "\tGK\t"
            alternativePosition = player.xpath(SoccerSpider.ALTERNATIVE_POSITION_LIST_VIEW).extract_first() #if position was an empty string, this will be the position
            classYear           = player.xpath(SoccerSpider.ACADEMIC_YEAR_LIST_VIEW).extract_first() #string of a class year e.g 'Freshman'
            home                = player.xpath(SoccerSpider.HOME_TOWN_LIST_VIEW).extract_first() #string of a single hometown e.g 'Easton, PA'
            highSchool          = player.xpath(SoccerSpider.HIGH_SCHOOL_LIST_VIEW).extract_first() #string of single high school e.g 'Tennent'
            previousSchool      = player.xpath(SoccerSpider.PREVIOUS_SCHOOL_LIST_VIEW).extract_first() #like highSchool
            height              = player.xpath(SoccerSpider.HEIGHT_LIST_VIEW).extract_first() #string with a single height element e.g ''5'10"'
            number              = player.xpath(SoccerSpider.NUMBER_LIST_VIEW).extract_first()
            weight              = player.xpath(SoccerSpider.WEIGHT_LIST_VIEW).extract_first()
            ...
            href                   = player.xpath(SoccerSpider.PROFILE_LINK_FROM_LIST_VIEW).extract_first()
        ...
        pages_to_visit = response.xpath('//select[@name = "ctl00$cplhMainContent$ddl_past_rosters"]/option/@value').extract()
       #format is list of strings [90, 91, 92...] which represent the rosters to visit (there were no hrefs to follow on page)

        num_pages_to_visit = SoccerSpider.CURRENT_ROSTER_YEAR - 2006 + 1 #goes as far back as 2006
        for i in range(1, num_pages_to_visit): #skipping index 0 because that is the starting page already parsed. Only parsing 10 years if applicable
            if(i < len(pages_to_visit)):
                ...
                yield scrapy.Request(url = urlToVisit, callback=self.parse_list)
