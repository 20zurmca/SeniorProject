...
class GPGSHolyCross(scrapy.Spider):

    name = 'GPGSSpiderHolyCross'
    ...
    start_urls = [
     'https://www.goholycross.com/fls/33100/statistics/m-soccer/' + year.split('-')[0] + '/teamcume.htm'
    ]
    ...
    def parse_list(self, response):
        ...
        roster_year      = int(GPGSHolyCross.year.split('-')[0])
    ...
    def get_potential_starts(self, response):
        ...
        return response.xpath('//table[@cellspacing = "0"]')[8].xpath('./tr')[-2] \
                                                                .xpath('./td[3]/font/text()') \
                                                                .extract_first() \
                                                                .strip() \

    def get_players(self, response):
        return response.xpath('//table[@cellspacing = "0"]')[8].xpath('./tr')[2:-2] \
               + response.xpath('//table[@cellspacing = "0"]')[9].xpath('./tr')[2:-3]

    def get_player_name(self, player):
        return player.xpath('./td[2]/font/a/text()').extract_first().strip()
