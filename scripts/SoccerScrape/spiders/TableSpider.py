...
class TableSpider(scrapy.Spider):
    ...
    def parse_list(self, response):
        ...
        for player in players:
            ...
            starting_year  = int(TableSpider.year.split('-')[0]) #current starting roster year
            ending_year    = int(TableSpider.year.split('-')[1])
            while(starting_year > 2006):
                ...
                index_of_current_roster_year = re.search('\d', response.url).start()
                length_of_current_roster_year = len(TableSpider.year)

                pages_to_visit.append(response.url[0:index_of_current_roster_year] #rebuild url and add
    ...
    def reference_index(self, school_url, attribute):
        ...
        return str(TableSpider.INDEX[school_url][attribute])
